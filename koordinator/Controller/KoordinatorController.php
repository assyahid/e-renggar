...
if(isset($_POST['submit']) && $_POST['submit'] == "Verifikasi usulan ini"){
	$data = $_POST;
	$query = "UPDATE usulan SET status='diverifikasi oleh Koordinator Tata Usaha',sent=4, posisi='Kepala BBLK Palembang' WHERE id_usulan=".$data['id_usulan'];
	$sql = mysqli_query($koneksi,$query);  
	if($sql){
...
if(isset($_POST['update']) && $_POST['update'] == "Validasi Usulan Barang Reagen"){
	...
	`status_reagen` = 'divalidasi oleh Koordinator Tata Usaha'
...
if(isset($_POST['update']) && $_POST['update'] == "Validasi Usulan Barang"){
	...
	`persetujuan` = 'divalidasi oleh Koordinator Tata Usaha'

  ...
  <div class="content">
    <div class="page-inner">
      <div class="page-header">
        <h4 class="page-title">Form Usulan Penghapusan Alat Kesehatan</h4>
...
<div class="row">
  <div class="col-md-12">
         ...
         <form method="POST" action="Controller/v1.php" enctype="multipart/form-data">
          ...
          <input type="hidden" name="id_usulan_barang" class="form-control" id="email2" value="<?=$_GET['id_usulan_barang']?>" placeholder="Input Merek">
             <div class="col-md-12 col-lg-12">
              ...
              <div class="col-md-12 col-lg-12">
                <div class="form-group">
               <input type="submit" name="submit" class="btn btn-default" value="Buat Usulan Penghapusan Alkes">
                </div>
          </form>
        ...
        <table id="multi-filter-select" class="display table table-striped table-hover" >
                      ...
                      <tbody>
                        <?php 
                        $usulan_barang=mysqli_query($koneksi,"SELECT * FROM penghapusan_alkes inner join usulan_barang on penghapusan_alkes.id_usulan_barang=usulan_barang.id_usulan_barang inner join barang on usulan_barang.id_barang=barang.id_barang WHERE penghapusan_alkes.id_usulan_barang = '".$_GET['id_usulan_barang']."' ")or die(mysql_error());
                        $no=1;
                        while($d=mysqli_fetch_array($usulan_barang)){ 
                          ...
                          <tr>
                            ...
                            <td><?= $d["justifikasi"] ?></td>
                            <td>
                              <?php if ($d['status_penghapusan_alkes']=="Pengajuan") { ?>
                            <a onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){ location.href='?hapus&id_penghapusan_alkes=<?php echo $d['id_penghapusan_alkes']; ?>&id_usulan=<?php echo $d['id_usulan']; ?>&id_usulan_barang=<?php echo $d['id_usulan_barang']; ?>' }" class="btn btn-icon btn-round btn-danger"> <i class="fa fa-trash"></i></a>
                              <?php } elseif($d['status_penghapusan_alkes']=="Revisi") { ?>
 ...
 <?php
    if(isset($_GET['hapus'])){
    $id_penghapusan_alkes=$_GET['id_penghapusan_alkes'];
    $id_usulan=$_GET['id_usulan'];
    $id_usulan_barang=$_GET['id_usulan_barang'];
$query2 = "DELETE FROM penghapusan_alkes WHERE id_penghapusan_alkes='$id_penghapusan_alkes'";
$sql2 = mysqli_query($koneksi,$query2); // Eksekusi/Jalankan query dari variabel $query
if($sql2){ // Cek jika proses simpan ke database sukses atau tidak  // Jika Sukses, Lakukan :  
?><script type="text/javascript">
    //<![CDATA[
    alert ("Berhasil Hapus");
    window.location="penghapusan_alkes.php?id_usulan=<?=$id_usulan?>&id_usulan_barang=<?=$id_usulan_barang?>";
    //]]>
  </script><?php
} else {  // Jika Gagal, Lakukan :  
  echo "Data gagal dihapus. <a href='penghapusan_alkes.php?id_usulan=<?=$id_usulan?>'>Kembali</a>";
}
  }
  ?>

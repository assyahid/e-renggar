...
$alkes=mysqli_query($koneksi,"SELECT * FROM alkes inner join barang on alkes.id_barang=barang.id_barang WHERE alkes.id_usulan = '".$_GET['id_usulan']."'")or die(mysql_error());
...
<div class="main-panel">
							...
							<div class="row align-items-center">
								...
								<div class="col col-stats ml-3 ml-sm-0">
									<div class="numbers">
										...
										<h4 class="card-title"><?= mysqli_num_rows($alkes); ?></h4>
										<!-- <a href="alkes.php">lihat data</a> -->
									</div>
								</div>
							</div>
						</div>
						...
						<div class="card-header">
							<h4 class="card-title"> Kebutuhan Alat Kesehatan
								<button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#exampleModal" style="float: right;">
									...
									<a type="button" target="_blank" href="../surat/cetak_alkes.php?id_usulan=<?= $id_usulan; ?>" class="btn btn-sm btn-success" style="float: right;margin-right: 5px;">
								...
								<div class="card-body">
									<div class="table-responsive">
										<table id="multi-filter-select" class="display table table-striped table-hover" >
											...
											<tbody>
												<?php 
												$no=1;
												while($d=mysqli_fetch_array($alkes)){ 
													?>
													<tr>
														<td><?=$no++;?></td>
														<td><?= $d["nama_barang"] ?></td>
														<td><?php $merek = mysqli_query($koneksi,"SELECT * FROM merek_alkes where id_alkes='$d[id_alkes]' ");
									...
									<span class="fas fa-plus"></span> <?= mysqli_num_rows($merek); ?></button>
														...
														<td><?= $d["justifikasi"] ?></td>
														<td><button type="button" class="btn btn-icon btn-round btn-primary" data-toggle="modal" data-target="#exampleEdit_<?php echo $d['id_alkes']; ?>">
															<i class="fa fa-edit"></i>
														</button>
														<a onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){ location.href='?hapus&id_alkes=<?php echo $d['id_alkes']; ?>&id_usulan=<?php echo $d['id_usulan']; ?>' }" class="btn btn-icon btn-round btn-danger"> <i class="fa fa-trash"></i></a>
													</td>
													</tr>	
	...
	<!-- Modal Edit Alkes-->
		<div class="modal fade" id="exampleEdit_<?php echo $d['id_alkes']; ?>" tabindex="-1" role="dialog">
				...
				<div class="modal-content">
					...
					<form method="POST" action="controller/v1.php">
						<div class="modal-body">
							...
							<input type="hidden" name="id_alkes" class="form-control" id="email2" placeholder="Input Jumlah" value="<?=$d['id_alkes']?>">
						...
						<div class="modal-footer">
							<input type="submit" name="update" class="btn btn-primary" value="Update Alkes">
...
<?php
		if(isset($_GET['hapus'])){
    $id_alkes=$_GET['id_alkes'];
    $id_usulan=$_GET['id_usulan'];
$query2 = "DELETE FROM alkes WHERE id_alkes='$id_alkes'";
			...
			<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
				<div class="modal-content">
					...
					<form method="POST" action="controller/v1.php">
						...
						<div class="modal-footer">
							<input type="submit" name="submit" class="btn btn-primary" value="Tambah Alkes">
							<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
						</div>
					</form>
				</div>
		...
		<div class="modal fade" id="exampleMerek" tabindex="-1" role="dialog">
			<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
				<div class="modal-content">
					...
					TAHUN 2022</b></p>
					<?php $alkes=mysqli_query($koneksi,"SELECT * FROM alkes inner join barang on alkes.id_barang=barang.id_barang WHERE alkes.id_usulan = '".$_GET['id_usulan']."'")or die(mysql_error());
					$show=mysqli_fetch_array($alkes);
					?>
					...
					<table class="table table-responsive">
						...
						<tbody>
							<?php 
							$alkes=mysqli_query($koneksi,"SELECT * FROM merek_alkes inner join alkes on merek_alkes.id_alkes=alkes.id_alkes")or die(mysql_error());
							$no=1;
							while($d=mysqli_fetch_array($alkes)){ 
								?>
								<tr>
									...
									<td><a onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){ location.href='?hapusmerek&id_merek_alkes=<?php echo $d['id_merek_alkes']; ?>&id_usulan=<?php echo $d['id_usulan']; ?>' }" class="btn btn-icon btn-round btn-danger"> <i class="fa fa-trash"></i></a></td>
								</tr>	
					...
					<form method="POST" action="controller/v1.php">
						<div class="modal-body">
							<input type="hidden" name="id_alkes" class="form-control" id="email2" placeholder="Input spesifikasi umum"value="<?=$show['id_alkes'];?>">

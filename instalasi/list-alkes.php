...
$id_usulan_alkes = $_GET["id_usulan_alkes"];
$usulan_alkes=mysqli_query($koneksi,"SELECT * FROM usulan_alkes WHERE id_usulan_alkes = $id_usulan_alkes")or die(mysql_error());
$data = [];
while($d=mysqli_fetch_array($usulan_alkes)){ $data[] = $d; }
...
<div class="main-panel">
		...
		<div class="page-inner">
			<div class="page-header">
				...
				<ul class="breadcrumbs">
					<a href="detail-usulan.php?id_usulan=<?=$_GET['id_usulan_alkes'];?>" class="btn btn-info">Kembali ke menu usulan</a>
				</ul>
			</div>
			<div class="row">
						...
						<div class="card-body">
							...
							<table class="table">
								<tr>
									<td>ID</td>
									<td>:</td>
									<td><?=$data[0]['id_usulan_alkes']; ?></td>
								</tr>
								<tr>
									<td>Nama Pengusul</td>
									<td>:</td>
									<td><?=$data[0]['id_user']; ?></td>
								</tr>
								<tr>
									<td>Tanggal Usulan</td>
									<td>:</td>
									<td><?=$data[0]['tgl_usulan_alkes']; ?></td>
								</tr>
								<tr>
									<td>Anggaran</td>
									<td>:</td>
									<td><select class="form-control" name="anggaran" required="">
							...
							<button class="btn btn-danger" type="submit" name="selesai">Kirim Usulan</button>
							</form>
						</div>
					</div>
				</div>
					...
					<div class="card">
						<div class="card-header">
							<h4 class="card-title"> Kebutuhan Alat Kesehatan
								...
								<?php if ($data[0]['posisi_usulan_alkes']==0) { ?>
								...
								<div class="card-body">
									<div class="table-responsive">
										<table id="multi-filter-select" class="display table table-striped table-hover" >
											...
											<tbody>
												<?php
												$query= mysqli_query($koneksi,"SELECT * FROM usulan_alkes_list inner join barang on usulan_alkes_list.id_barang=barang.id_barang"); 
												$no=1;
												while($d=mysqli_fetch_array($query)){ 
													...
													<tr>
														<td><?=$no++;?></td>
														<td><?= $d["nama_barang"] ?></td>
														<td><?php $merek = mysqli_query($koneksi,"SELECT * FROM merek_alkes where id_usulan_alkes_list='$d[id_usulan_alkes_list]'");
									...
									<span class="fas fa-plus"></span> <?= mysqli_num_rows($merek); ?></button> -->
										<a href="merek-alkes.php?id_usulan_alkes_list=<?=$d['id_usulan_alkes_list']?>&id_usulan_alkes=<?=$id_usulan_alkes?>" class="btn btn-sm btn-primary"><span class="fas fa-plus"></span> <?= mysqli_num_rows($merek); ?></a>
														 </td>
													...
													<?php	} ?>  </td>
														...
														<td>
															...
															<button type="button" class="btn btn-icon btn-round btn-primary" data-toggle="modal" data-target="#exampleEdit_<?php echo $d['id_usulan_alkes_list']; ?>">
															<i class="fa fa-edit"></i>
														</button>
														<a onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){ location.href='?hapus&id_usulan_alkes_list=<?php echo $d['id_usulan_alkes_list']; ?>' }" class="btn btn-icon btn-round btn-danger"> <i class="fa fa-trash"></i></a>
	...
	<!-- Modal Edit usulan_barang-->
		<div class="modal fade" id="exampleEdit_<?php echo $d['id_usulan_alkes_list']; ?>" tabindex="-1" role="dialog">
		...
		<div class="modal fade" id="exampleModal"  role="dialog">
			<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
				<div class="modal-content">
					...
					<form method="POST" action="controller/v1.php">
						<div class="modal-body">
							<input type="hidden" name="id_usulan_alkes" class="form-control" id="email2" placeholder="Input spesifikasi umum"value="<?=$_GET['id_usulan_alkes'];?>">
                ...
                <span class="fas fa-plus"></span> Tambah Data Barang</button>
						...
						<div class="modal-footer">
							<input type="submit" name="submit" class="btn btn-primary" value="Tambah Usulan Alkes">
							<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
						</div>
					</form>
				</div>

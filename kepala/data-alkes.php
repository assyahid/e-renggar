...
$usulan_barang=mysqli_query($koneksi,"SELECT * FROM usulan_barang inner join barang on usulan_barang.id_barang=barang.id_barang inner join users on usulan_barang.id_user=users.id_user WHERE usulan_barang.kategori='Alkes'")or die(mysql_error());

$id_usulan_barang=mysqli_query($koneksi,"SELECT * FROM usulan_barang inner join barang on usulan_barang.id_barang=barang.id_barang WHERE usulan_barang.kategori='Alkes'")or die(mysql_error());
$barang   =mysqli_query($koneksi,"SELECT * FROM barang where kategori='Alat Kesehatan' ORDER BY nama_barang ASC")or die(mysql_error());



$pegawai=mysqli_query($koneksi,"SELECT * FROM pegawai ")or die(mysql_error());
$surat=mysqli_query($koneksi,"SELECT * FROM usulan ")or die(mysql_error());
							...
							<div class="row align-items-center">
								...
								<div class="col col-stats ml-3 ml-sm-0">
									<div class="numbers">
										...
										<?php $perbaikan=mysqli_query($koneksi,"SELECT * FROM perbaikan_alkes")or die(mysql_error()); ?>
								...
								<div class="col col-stats ml-3 ml-sm-0">
									<div class="numbers">
										<p class="card-category">Data Penghapusan Alkes</p>
											<?php $penghapusan=mysqli_query($koneksi,"SELECT * FROM penghapusan_alkes ")or die(mysql_error()); ?>
									...
									<span class="fas fa-plus"></span> <?= mysqli_num_rows($merek); ?></button> -->
										...
										<?php $mereks = mysqli_query($koneksi,"SELECT * FROM merek_barang where id_usulan_barang='$d[id_usulan_barang]' and status_merek='disetujui'");
													...
													<?php	} ?>  </td>
														...
														<td><?= $d["justifikasi"] ?></td>
														<td>
															<?php if ($d['persetujuan']!="divalidasi oleh Kepala BBLK Palembang" && $d['persetujuan']=="divalidasi oleh Koordinator Tata Usaha") { ?>
			...
			<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title">Validasi Data Kebutuhan Alat Kesehatan</h5>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
							<span aria-hidden="true">&times;</span>
						</button>
					</div>
					<form method="POST" action="controller/KepalaController.php">
						...
						<div class="col-md-12 col-lg-12">
								<div class="form-group">
									...
									<input type="number" name="jumlah_tersedia" class="form-control" id="email2" placeholder="Input Jumlah" value="<?=$d['jumlah_tersedia']?>" readonly>
								</div>
							</div>
							...
							<div class="col-md-12 col-lg-12">
								<div class="form-group">
									...
									<select class="form-control form-control" name="kondisi" id="defaultSelect" readonly disabled="">
									...
									<?php if ($d["kondisi"]=='Rusak Ringan') { ?>
																<?php $perbaikan_alkes = mysqli_query($koneksi,"SELECT * FROM perbaikan_alkes where id_usulan_barang='$d[id_usulan_barang]'");
															while ($pa=mysqli_fetch_array($perbaikan_alkes)) { ?>
																<a href="../image/<?=$pa['file']?>" class="btn btn-sm btn-warning" target="_BLANK"><i class="fa fa-file"></i> lihat surat usulan perbaikan</a>
															<?php }  ?>
																<?php } elseif ($d["kondisi"]=='Rusak Berat') {?>
																			<?php $penghapusan_alkes = mysqli_query($koneksi,"SELECT * FROM penghapusan_alkes where id_usulan_barang='$d[id_usulan_barang]'");
															while ($pha=mysqli_fetch_array($penghapusan_alkes)) { ?>
																<a href="../image/<?=$pha['file']?>" class="btn btn-sm btn-danger" target="_BLANK"><i class="fa fa-file"></i> lihat surat penghapusan</a>
															<?php } } ?>
								</div>

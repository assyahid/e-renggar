...
if(isset($_POST['submit']) && $_POST['submit'] == "Verifikasi usulan ini"){
	$data = $_POST;
	$query = "UPDATE usulan SET status='diverifikasi oleh Kepala BBLK Palembang',sent=5, posisi='Sudah di Verifikasi Semua' WHERE id_usulan=".$data['id_usulan'];
	$sql = mysqli_query($koneksi,$query);  
	if($sql){
...
if(isset($_POST['update']) && $_POST['update'] == "Validasi Usulan Barang Reagen"){
	...
	`status_reagen` = 'divalidasi oleh Kepala BBLK Palembang'
...
if(isset($_POST['update']) && $_POST['update'] == "Validasi Usulan Barang"){
	...
	`persetujuan` = 'divalidasi oleh Kepala BBLK Palembang'
